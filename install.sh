...
# 获取 install.sh 所在目录的绝对路径
IPTABLES_HOME="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
...
# 安装脚本
echo "[4/7] 安装防火墙脚本..."
if [ -f "$IPTABLES_HOME/scripts/iptables-docker.sh" ]; then
    # 从当前目录安装
    cp "$IPTABLES_HOME/scripts/iptables-docker.sh" "$INSTALL_DIR/$SCRIPT_NAME"
	cp "$IPTABLES_HOME/scripts/awk.firewall" "$INSTALL_DIR/awk.firewall"
	cp "$IPTABLES_HOME/scripts/parse_yaml.py" "$INSTALL_DIR/parse_yaml.py"
